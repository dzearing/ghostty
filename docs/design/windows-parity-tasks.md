...
1. **T40** — DONE 2026-07-14 (lost renderer wakeups; see row). The fix is
   on the branch but NOT in the installed release yet — the next release
   refresh (T36 flow, `-Dstrip=false`) delivers it to the user.
2. **T49** — hero mode broken on Windows (regression vs T19's validation).
3. **T50** — rename window (ctrl+shift+r) needs a proper dialog.
4. **T48** — deadlock root-cause, once a symbolized watchdog dump exists
   (safeguards already in place; see the T48 row).
