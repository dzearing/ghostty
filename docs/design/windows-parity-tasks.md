...
**Current priorities (user directive 2026-07-14, overrides table order):**
the goal is a *highly reliable, highly performant* Windows client where the
Claude Code experience is great. Work these first, in order, before falling
back to first-todo-in-table order:

1. **T40** — DONE 2026-07-14 (lost renderer wakeups; see row). Release
   refresh delivering the fix was launched 2026-07-14 late evening
   (detached upgrade script, staging = HEAD ReleaseFast + pdb); the next
   session should verify the install answered `+list` and tell the user.
2. **T49** — investigated 2026-07-14: NO repro on HEAD (Debug + Release,
   new hero-mode.ps1 oracle ALL PASS). Blocked on user re-test after the
   refresh; see row.
3. **T50** — rename window (ctrl+shift+r) needs a proper dialog.
4. **T48** — deadlock root-cause, once a symbolized watchdog dump exists
   (safeguards already in place; see the T48 row). The refreshed install
   now has a matching pdb, so the next watchdog dump WILL be symbolizable.
