...
1. **T40** — Claude Code scrolling is "atrociously slow" on the release
   build (user's words, 2026-07-14). Performance of the busy-TUI path is
   the top item.
...
| T40 | FIX PERF: scrolling in Claude Code TUI on the release build is "atrociously slow. really bad" (user, 2026-07-14; earlier: jerky). HIGH PRI — top of the priority list. Suspects to profile, not guess (measure first with the symbolized build): per-keystroke GUI-thread acquisition of renderer_state.mutex vs PTY-reader processOutputLocked contention (see T48 candidate 2), renderer frame pacing under heavy output, WM_ERASEBKGND/paint churn, ConPTY read chunk sizing (src/termio/Exec.zig threadMainWindows). Deliverable: measurably smooth scrolling while a Claude Code turn streams output; before/after numbers (e.g. frame interval or paint counts) recorded here | I | — | todo | — | — |
