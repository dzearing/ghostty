...
3b. **T113 IS NOW FIRST (user-reported outage, 2026-07-27).** It was already
   next in the chain above, but its justification changed: it is not a
   documentation-contract nicety, it is why the user's ghoztty-plugin banner
   hooks silently do nothing on Windows (`resolve_pane` needs
   `$GHOZTTY_PANE_ID`, or a tty for either fallback, and an agent-backed pane
   has neither). Implementation is ALREADY committed as WIP `b86dce1d0`; what
   remains is validation — `test/win32/pane-id.ps1` plus the new end-to-end
   check that a fresh session's banner hooks update the banner with no plugin
   edit. Then **T129** (banner editor undiscoverable — same user report, same
   subsystem, cheap) is a natural follow-on in the same area.
