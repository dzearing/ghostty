...
20. **PUBLISH-READINESS QUEUE (user directive 2026-07-20, standing): work
    ...
    (visible-relaunch scrollback loss — REAL-WORLD restore path, do before or
    with T89i) → **T89i** (session-persistence E2E hardening
...
| T106 | FIX: VISIBLE relaunch loses the re-attached pane's replayed scrollback — found by T105 validation (2026-07-20, pre-existing): same manifest, a `-WindowStyle Minimized` relaunch re-attaches with full ring-replay scrollback (F8 green), but a normal visible relaunch leaves the split pane's terminal EMPTY (`+read` returns just the prompt even at `--lines=2000`; sessions ATTACH fine per F7; grab-independent, fix-independent — isolated on both binaries). Real relaunches are visible, so users lose scrollback the harness's minimized cycle never sees. Suspect the resize/repaint path during visible restore clearing the just-replayed core scrollback (ConPTY repaint w/ erase, or replay-vs-resize ordering). Repro: flip session-reattach.ps1's F5 relaunch to visible and watch F8 | K | T89f2 | todo | — |
