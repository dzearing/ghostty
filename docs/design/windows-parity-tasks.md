...
2. **T20 → T21 → T22** — remote windows on Windows: user explicitly needs
...
3. **T48** — deadlock root-cause. The refreshed install has a matching
   pdb, so the next watchdog dump WILL be symbolizable. Adversarial
   investigation applies (three ranked candidates in the row).
4. **T53** — long-context reliability + perf soak/tuning pass (new row).
5. **T52** — build provenance surfaced in-app (new row; the 2026-07-15
   "no parity" report was a July-5 exe — make "which build is this"
   answerable at a glance).
6. **T51** — full parity re-audit (new row). Deliberately LAST in the
