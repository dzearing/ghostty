...
| T100 | FIX (delivery blocker): `zig build agent` fails to LINK the ghoztty-agent EXE on the box — `lld-link: undefined symbol: WinMain` (`libcmt.lib(exe_winmain.obj)`). The agent sets `exe.subsystem = .Windows` (GhosttyAgent.zig:83, so no console window pops for the tray daemon) but has `pub fn main` + MSVC libc, and this Zig (0.15.2) + MSVC config selects the CRT's WinMain entry which is never provided. Found by T89f2 (2026-07-20); PRE-EXISTING (T89f2 touches only win32 app files) — a stale working agent exe from earlier today let T89f2's on-box test run, but a fresh `zig build agent` is red. Deterministic (repro'd ×2). NOT in the parity validation lanes (`test-agent` builds console TEST binaries, still green), but BLOCKS T89h (agent must ship in the release zip/MSI + all 3 install locations). Likely fix: provide a `wWinMain`/`WinMain` shim (or a `-e wWinMainCRTStartup`/mainCRTStartup entry) for the Windows agent exe, or drop `.subsystem=.Windows` and hide the console another way (FreeConsole at startup). Candidate for the Mac seat if it's an upstream/toolchain interaction | K | — | todo | — |
