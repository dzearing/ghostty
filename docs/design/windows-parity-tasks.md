...
| T89d | Win32 surfaces OPEN under local agent: LocalAgent.zig find-or-spawn (2s bound, 15s cooldown, exec fallback), consume session-persistence, remote_conn injection (pinned, env, T84 ^C flag); `session-open.ps1`. NOTE (from T89c): the per-user single-instance mutex is mode-independent, so a local-pipe agent and a relay agent currently can't coexist — the local agent needs a DISTINCT single-instance identity (mode/dir-keyed mutex) so find-or-spawn doesn't fight the relay agent (T89a decision 2 = separate instances) | K | T89c | todo | — |
