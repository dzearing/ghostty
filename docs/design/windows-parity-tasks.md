...
one line â€” put detail in `windows-parity-details.md`.
...
1. Read `go.md` and this doc ONLY. Do NOT read the details/log/audit/spec
   docs wholesale â€” they are split out precisely to keep resume cheap.
...
5. Run the task's **Validation** (in its details section). Do not mark
   `done` on a clean build alone â€” validation must actually pass, on the
   box when it says so.
6. Update: the state table row (status + commit hash â€” ONE line), the
...
Keep tool output small â€” grep build logs for `error`, take only the last
...
validation become tasks, not loose threads). Never delete a task â€” mark
...
Windows-native. Not slow, not crashing â€” well tuned and well tested.
...
1. ~~T50~~ â€” DONE 2026-07-15 (real "Rename Window" dialog).
2. ~~T54~~ â€” DONE 2026-07-15 (this doc restructure; resume read is now
   small).
3. **~~T20~~ â†’ ~~T21b~~ â†’ ~~T21a~~ â†’ ~~T22~~** â€” remote windows on Windows: T20
   ...
   design) â†’ T22b (Zig device-directory client) â†’ T22c (win32 chooser dialog
   ...
   `ipc-machine-chooser.ps1` ALL PASS on-box â€” real chord â†’ chooser opens â†’
   `GET /v1/client/devices` â†’ Escape-close, no crash). The T22 remote-window
   series is complete; remaining Phase-G follow-ups are T56 (reconnect) and
   T42 (remote env/PATH). Next in this priority list: T58.
4. **~~T48a~~ â†’ ~~T48~~** â€” deadlock. **T48 (fix) DONE 2026-07-15**
   ...
   ghoztty pdb needed). NOT a lock cycle â€” the GUI thread calls `SetFocus`
   ...
   can pump.
5. **~~T58~~ â†’ ~~T59a~~ â†’ ~~T59b~~** â€” hero mode TRUE port (user, 2026-07-16,
   ...
   snapshot pipeline (offscreen-target capture â€” hidden panes capture
   ...
   clean (thumbnail heartbeat â‰ˆ7fps/renderer, no stalls); hero-mode.ps1
   ALL PASS (58 assertions, incl. a mid-slide oracle); both test lanes +
   P1â€“P3 green. The hero-mode TRUE port is COMPLETE. Next: T53.
6. ~~T53~~ â€” COMPLETE 2026-07-17. T53a DONE (soak harness; found+fixed
   the WM_APP_WAKEUP queue flood). T62/T63 DONE (pty read batching;
   +close join race). **T53b DONE 2026-07-17**: the detached 180-min
   soak finished ALL PASS (11) â€” zero leak growth (private +0.5MB,
   handles/GDI/USER +0 q1â†’q4), responsive at all 720 samples, 180/180
   echo probes median 248ms, median fps 59; only WARN was the known
   T62 stall (binary predated the fix). Interactive profiling
   (`profile-latency.ps1`, ALL PASS 14): keyboard latency 65â†’81ms at
   0â†’150k scrollback lines, GUI-thread RTT 0ms through seek bursts
...
7. ~~T52~~ â€” DONE 2026-07-18: `ghoztty +version` now answers "which build
   is this window running?" from any pane (new IPC `version` verb â†’
   "Running Instance" section with version/commit/mode/exe/modified/pid);
   `+list --json` carries the same as `data.build` (additive â€” Mac golden
   shape untouched); command palette gained "About Ghoztty". Validated by
   `test/win32/ipc-version.ps1` ALL PASS (22) three runs in a row; P1â€“P3
   + both test lanes green. Next: T51.
8. **T51** â€” full parity re-audit. Deliberately LAST in the queue per the
...
|----|------|-------|------|--------|---------|
| T01 | Verify fresh ZIP keybinds on box | A | â€” | todo | â€” |
| T02 | Keybind gaps: ctrl+p, ctrl+f4 | A | â€” | done | 82e096f4b |
| T03 | Named-pipe client helper + CLI un-guard | B | â€” | done | 353d70abf.. |
...
| T18 | `swap_split` on win32 | F | â€” | done | see details |
| T19a | Hero mode design (win32) | F | T18 | done | see details |
| T19 | Hero mode on win32 (implement) (CORRECTION 2026-07-16: shipped a static geometric stand-in, NOT the Mac design â€” see T58/T59) | F | T19a | done | f37bd1e3c |
...
| T23 | MSI fix â†’ uninstall entry works | H | â€” | todo | â€” |
| T24 | Windows release channel + update check | H | T23 | todo | â€” |
| T25 | Full conformance checklist (spec Â§8) | â€” | T17,T19,T21a | todo | â€” |
| T26 | OS color-scheme sync | I | â€” | done | see details |
| T27 | PowerShell shell integration | I | â€” | done | see details |
| T28 | Minor action no-ops cleanup | I | â€” | in-progress | see details |
| T29 | Mac-side: action fallthroughs to showChildExited | I | â€” | todo | â€” |
| T30 | Mac-side: IPC dial must not modal-block | I | â€” | todo | â€” |
| T31 | `+list --pid` + real pid leaf data | I | T05 | done | see details |
| T32 | Split IpcServer.zig; pure logic + unit tests | J | â€” | done | 640457b0d.. |
| T33 | Native win32 test lane | J | T32 | done | see details |
| T34 | Windows shell types, first-class | J | â€” | done | see details |
| T35 | Sticky pane banner on win32 | I | â€” | todo | â€” |
| T36 | Release install refresh flow | H | â€” | in-progress | ae71b19b4.. |
| T37 | CLAUDE.md symmetry mandate + dual-arch instructions | â€” | â€” | todo | â€” |
| T38 | Windows build in the release process | H | T23,T24 | todo | â€” |
| T39 | Website: Windows installer download link | H | T38 | todo | â€” |
| T40 | FIX PERF: lost renderer wakeups (slow scrolling) | I | â€” | done | see details |
| T41 | Skip close-confirm when shell is idle | I | â€” | todo | â€” |
| T42 | Remote sessions: user env/PATH missing | G | â€” | todo | â€” |
| T43 | Proper visual debug banner on win32 | I | â€” | todo (lower priority) | â€” |
| T44 | FIX CRASH: rename overlay, single-tab window | I | â€” | done | 7510d2cd2.. |
...
| T47 | ctrl+k â†’ clear_screen keybind | I | â€” | done | see details |
| T48a | Root-cause the release GUI deadlock (dump analysis) | I | â€” | done | see details |
| T48 | FIX DEADLOCK: defer SetFocus out of WindowProc (re-entrant IME/CTF hang) | I | T48a | done | e35ef81fd |
| T49 | Hero-mode regression report â†’ stale binary (CORRECTION 2026-07-16: the user's actual repro was the command palette, not the keybind â€” see T57) | F | T19 | done | c795455ff.. |
| T50 | Real "Rename Window" dialog | I | T44 | done | 39988009a |
| T51 | Full parity RE-AUDIT | â€” | T50,T22c,T48,T53 | todo | â€” |
| T52 | Build provenance visible in-app: IPC `version` verb, `+version` "Running Instance" section, `+list --json` data.build, palette "About Ghoztty" box (shared win32 provenance.zig); `ipc-version.ps1` ALL PASS 3x | I | â€” | done | (this commit) |
...
| T62 | FIX: +read stalls many seconds (16s observed) while a pane floods tiny writes â€” renderer-mutex starvation on the GUI/IPC path (T48 static candidate 2, now reproduced). Fixed: read-thread batching (64KB + pipe top-up), one lock cycle per batch; 80â€“127ms post-fix. See details | I | T53a | done | 5562c65ab |
| T63 | FIX: +close of a noisy window hung the GUI thread forever â€” Exec.threadExit's one-shot CancelIoEx missed while the reader parsed, join() never returned (found by the T62 validation run, 9+ min hang observed). Fixed with T62: quit-byte check before every blocking read + retrying cancel; +close now asserted <10s in ipc-under-load.ps1 (277ms). See details | I | T62 | done | 5562c65ab |
| T54 | Resume-doc diet (this restructure) | â€” | â€” | done | 6968d82e7 |
| T55 | FIX: hero-mode.ps1 fails on HEAD (chords not dispatched) â€” root cause was the TEST's positive control: ctrl+shift+r now opens the T50 modal rename dialog, which disables the owner window and silently ate every later chord. Control switched to ctrl+k (clear_screen, no UI left behind); not a key-path regression | F | T19 | done | (this commit) |
| T60 | FIX: window title jitters a few px left/right on a timer while busy (user, 2026-07-16; row renumbered from a duplicate T56 on 2026-07-16). Likely cause: Claude Code's title spinner â€” the braille glyphs (â /â ‚/â€¦) come from a FALLBACK font (MS Gothic per app log) with per-glyph advance widths, so the centered title re-centers to a different width every spinner frame. Investigate where the win32 tab/title text is drawn (Window.zig caption/tab paint); candidate fixes: left-align the title, reserve a fixed-width cell for the leading glyph, or measure/center on the title minus the spinner char | I | â€” | todo | â€” |
| T57 | FIX: "Toggle Hero Mode" (and other fork actions) missing from the win32 command palette â€” the REAL cause of the T49 user report ("no hero mode in command palette", 2026-07-16): the palette is a hardcoded static list (Surface.zig palette_entries), never updated with fork actions, so hero mode was undiscoverable even though the keybind worked. Added: Toggle Hero Mode, Swap Split Right/Down/Left/Up, Rename Window (prompt_surface_title). Skipped prompt_surface_banner (win32 no-op until T35). hero-mode.ps1 grew a palette section: ctrl+shift+p â†’ type "hero" â†’ Enter â†’ hero geometry asserted. Consider (T51 audit): generate palette entries from core command.zig defaults instead of a parallel list, so this class of drift can't recur | F | T19 | done | (this commit) |
| T56 | Remote reconnect on win32 (WP-D1 parity) | G | T21b | todo | â€” |
| T58 | Hero mode TRUE port â€” design (win32): Mac hero = animated hero strip + snapshot thumbnail carousel + drag divider; T19 shipped a static live-pane stand-in (user, 2026-07-16). Decided: renderer-side FBO-downscaled snapshots (pre-swap hook in generic.zig; HWND capture rejected â€” child GL windows have no DWM surface), non-hero panes SW_HIDE + renderer kept awake + all hero-sized (no reflow on swap), owner-painted carousel in new HeroCarousel.zig, snapshot-slide animation, per-tab ratio + divider drag. T59 split â†’ T59a/T59b | F | T19 | done | (this commit) |
| T59a | Hero mode TRUE port â€” snapshot pipeline + static carousel. Spike outcome: capture reads the OFFSCREEN render target (OpenGL.captureThumb), so hidden panes capture cleanly â€” no fallback needed. Renderer hook in generic.zig; Surface snap buffer/DIB + WM_APP_HERO_SNAP; SW_HIDE + renderer-awake layout, all leaves hero-sized; owner-painted HeroCarousel.zig + hero_math.zig (unit tests in both lanes); click-select; per-tab ratio field; hero-mode.ps1 rewritten (DPI-aware harness) | F | T58 | done | a859c9976 |
| T59b | Hero mode TRUE port â€” interactions/motion: wheel scroll, divider drag + per-tab ratio, hover chrome, slide + re-center animations, reduced-motion, GHOZTTY_PERF check, screenshot | F | T59a | done | 5a10762ed |
...
| T64 | FIX: SendInput-unicode (VK_PACKET) text injection silently dropped â€” screen readers/on-screen keyboards/automation typed nothing into panes (found by the T53b profiling harness; both input modes affected). See details | I | â€” | done | 3cb802605 |
...
- `src/apprt/win32/` IPC (as of 2026-07-12): IpcServer.zig (pipe
  ...
  (args.zig, list.zig â€” unit tested in both lanes). No action stubs remain
  (T19 landed the last one). `startUpdateCheck` still hard-disabled (T24).
- `src/apprt/none.zig` `sendIpc` â€” Mac client wire protocol (4-byte BE length
  + JSON), reuse `src/apprt/ipc.zig` `parseResponse`.
- `macos/Sources/Features/IPC/IPCServer.swift` â€” reference server semantics
  (registry, idempotency, send-keys notation, list format, rearrange).
- `macos/Sources/Features/Terminal/BaseTerminalController.swift` â€” activity
  title suffix + titleOverride precedence.
- `src/remote/` â€” shared Zig remote core (ws_client, relay_dial, client_mux,
  ssh_transport); termio `.remote` backend. Swift supplies only UI + creds.
- `src/config/Config.zig` ~line 6880 â€” the Windows ctrl-mirror keybind block.
- Windows keyboard path: `src/apprt/win32/Surface.zig` `handleKeyEvent`.

## Related docs (read only the slice you need)

- `windows-parity-details.md` â€” per-task spec/validation/evidence sections
  (`## T<id>`), plus "Bootstrap & environment" and the backlog. Read ONLY
  your task's section.
- `windows-parity-log.md` â€” dated session log, newest first. Open a single
  entry only when you need the backstory for your current task. Append ONE
  short entry at every task boundary.
- `windows-parity-audit.md` â€” the 2026-07-12 three-way audit findings.
- `windows-parity-spec.md` â€” architecture decisions (pinned). Read its
