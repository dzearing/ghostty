...
| T89g | Tombstone RELAUNCH floor on win32 — DONE 2026-07-20 (see priority-list item 18 + details §T89g): the tombstone/RELAUNCH machinery is OS-agnostic; the one win32 gap was `App.restoreSessionLayout` attaching only `alive==true` ids so relaunchable tombstones nulled → fresh OPEN. Fix: propagate wire `relaunchable` into `connection.OwnedSession`; forward alive-or-relaunchable ids to ATTACH (`attach_set`; `alive`→`attach` rename across 5 restore helpers) so shared termio fires RELAUNCH per `session-relaunch`. `session-relaunch.ps1` ALL PASS (19) ×3 (auto: same id RELAUNCHed + divider + pre-kill ring scrollback precedes it; prompt: live tombstone + press-any-key affordance → keystroke fires deferred relaunch). Both lanes + test-agent + P1–P3 green | K | T89f | done | (this commit) |
| T101 | FIX (user, 2026-07-20 live review): the win32 sticky pane banner overlay OCCLUDES terminal content — the terminal scroll area renders BEHIND the layered banner strip, so the banner covers the top content rows. Root cause: `Surface.handleResize` passes the FULL pane height to `core_surface.sizeCallback` (Surface.zig:2387); the banner height is never subtracted and there is no top offset, so the grid fills the whole pane under the floating overlay. Mac reserves space via the SwiftUI VStack (banner above terminal). Fix: inset the terminal drawable region from the top by the banner strip height (dynamic — re-inset when the banner is set/grows/collapses/cleared and on DPI change); verify content starts below the banner. Validate via pane-banner.ps1 (assert the grid top ≥ banner bottom) + visual | I | T35 | todo | — |
| T102 | FEATURE (user, 2026-07-20 live review): win32 right-click on a pane PASTES (Windows-Terminal-style) instead of opening a context menu — build a Mac-parity right-click context menu (reference the macOS pane context menu: Copy, Paste, Split Right/Down, Clear, Change Title, Background Color…, etc.), native win32 `TrackPopupMenu` with T79 dark-mode, wired to the same actions as the command palette. Include changing the pane background color (ties to T67 `--color`/ChooseColorW). Decide right-click-paste disposition (Mac has none; parity = menu, possibly a config opt-out). Validate via a win32 script + visual | I | T67,T79 | todo | — |
| T100 | FIX (delivery blocker): the **native-MSVC GUI-subsystem** link is broken on the box — `lld-link: undefined symbol: WinMain` (`libcmt.lib(exe_winmain.obj)`). Hits ANY GUI-subsystem exe built for `native-native-msvc`: `zig build agent` (agent sets `exe.subsystem=.Windows`, GhosttyAgent.zig:83) AND `zig build -Doptimize=ReleaseFast` for the APP itself (ReleaseFast uses the Windows subsystem; Debug uses Console so it links — that's why the debug lane + test-agent stay green). Root: `pub fn main` + MSVC libc, this Zig (0.15.2) selects the CRT's WinMain entry which is never provided. Found by T89f2 (2026-07-20); PRE-EXISTING (T89f2 is app-side only) — and REGRESSED since Jul 19 (that day's msvc-native ReleaseFast staging built fine; likely a zig-cache/toolchain state change, since no entry/build code changed). **WORKAROUND (proven 2026-07-20): build the release with `-Dtarget=x86_64-windows-gnu`** (the GNU ABI has no libcmt exe_winmain.obj) — this is exactly what `publish-windows-release.ps1` already does, and it built a clean HEAD ReleaseFast staging (d15bb230c, pdb present) used for the on-box upgrade. So RELEASE delivery is unblocked via the gnu target; only the native-msvc GUI link is red. T89h still needs the AGENT built — build it with `-Dtarget=x86_64-windows-gnu` too, or add a `wWinMain`/entry shim for native-msvc. Candidate for the Mac seat (toolchain interaction). NOT in the parity validation lanes | K | — | todo | — |
