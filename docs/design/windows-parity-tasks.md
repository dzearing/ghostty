...
20. **PUBLISH-READINESS QUEUE (user directive 2026-07-20, standing): work
    ...
    reproduces a real IPC starvation on the agent path) → **T111 (DO THIS
    NEXT — publish blocker)**: `+list` answers **1/40** under a double
    agent-backed storm and the CLI gets `No running Ghoztty instance
    found.` — the GUI-thread pipe listener stops accepting, i.e. the
    user-visible "Not Responding" freeze, on the path EVERY local pane now
    uses (persistence is default-on). Prime suspect + first A/B are in the
    row → **T38** (Windows build in the release process; version+arch
