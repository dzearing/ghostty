...
17. ~~T89e~~ — DONE 2026-07-20 (close-vs-quit: user close ⇒ CLOSE via
   ...
   window is; the IPC override baton suppresses `buildRemoteInherit`). **Next
   on-box: T99** (unblocks agent-backed IPC splits, which T89f re-attach
   needs) **→ T89f** (folds with T96's production pty teardown for the close
   path). T29/T30/T87 remain Mac-seat.
