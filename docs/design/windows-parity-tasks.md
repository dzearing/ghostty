...
20. **PUBLISH-READINESS QUEUE (user directive 2026-07-20, standing): work
    ...
    T111b row → **T38** (Windows build in the release process; version+arch
...
| T111b | REMAINING agent-path IPC starvation after T111a — `session-persistence.ps1` section E still red on 4 asserts: **E2 15/40**, **E3/E4 `+read` of the echo-storm pane 9202ms** (T62 bound 2s), **E5 quiet-pane round-trip**. A–D + E1 + E6–E12 all PASS. Two leading hypotheses, both unproven — INSTRUMENT BEFORE FIXING (T111a's prime suspect was refuted by measurement): **(1) mutex barging** — T111a bounded each hold to ~80ms (debug build), so a 9.2s wait is ~100 CONSECUTIVE lost races; the drain unlocks and re-locks with essentially zero gap (just a slice-pointer bump), where Exec has PeekNamedPipe/ReadFile syscalls between its unlock and next lock, giving a parked waiter a real window. Candidate fix: `std.Thread.yield()` between slices in `feedSliced` (cheap when uncontended; SwitchToThread). **(2) something GLOBAL, not per-pane** — E5 round-trips the QUIET pane `spC`, whose mutex is free, so a pure per-pane-mutex theory does NOT explain it; suspect the agent-connection WRITE path (`+send-keys` to an agent-backed pane writes to the one duplex pipe the storm is saturating inbound) or serialization behind an earlier still-in-flight request (the IPC server is a SINGLE pipe instance served serially, so a 9s handler blocks the next client at PIPE_BUSY). Also available and worth doing regardless: core already delivers a `.pwd` apprt action on `pwd_change` (`Surface.zig` `.pwd_change` → `performAction(.pwd)`) which win32 currently ACKS AND DROPS (`App.zig` "acknowledge actions that don't need Win32-specific handling") — caching it there makes `+list`, the liveness probe, take no terminal lock at all. Repro harness: `scratchpad/t111-repro.ps1` (fast 2-pane A/B, ~3min) is NOT sensitive enough — it tops out at 767ms; extend it to E's shape (5 idle + 2 storm agent-backed panes, plus `+read`/round-trip probes) for a fast loop, then confirm on the real harness. PUBLISH BLOCKER | K | T111a | todo | — |
