...
struct NewTerminalIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<TerminalEntity?> {
        ...
        if let parentParam = self.parent {
            ...
            parent = preferred.focusedSurface ?? preferred.surfaceTree.root?.leftmostLeaf().surfaceView
        } else {
            parent = nil
        }
        ...
        case .window:
            ...
            if let view = newController.surfaceTree.root?.leftmostLeaf().surfaceView {
