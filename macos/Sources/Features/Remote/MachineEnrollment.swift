...
protocol MachineEnrollmentRevoking: AnyObject {
    ...
    /// Give this machine back to `accountEmail` on sign-in: cancel a pending
    /// revocation for that same account, or re-enroll a suspended enrollment.
...
/// What `revokeForSignOut` did. Every case except `.unreachable` means the
...
enum MachineEnrollmentRevocation: Equatable {
    ...
    /// The relay could not be reached, so THE MACHINE IS STILL REACHABLE.
...
final class LocalMachineEnrollment: MachineEnrollmentRevoking {
    ...
    private func revokeCore(accountEmail: String) async -> MachineEnrollmentRevocation {
        guard let credential = envFile.load()?.credential else {
            // Nothing enrolled here: a stale suspended record can't apply.
        ...
        do {
            ...
            // A malformed RELAY_BASE names no relay we could ever revoke
            // against. The credential is unusable by the agent too, so clear
            // it and report the machine secured rather than blocking sign-out
            // on something no retry can fix.
            Ghostty.logger.warning(
                "machine enrollment: unusable RELAY_BASE in relay.env; clearing the local credential")
            envFile.delete()
            return .alreadyRevoked
        }

        let identity: RelayDeviceClient.Identity?
        do {
            identity = try await client.whoami(deviceToken: credential.deviceToken)
        } catch {
            return .unreachable(detail: error.localizedDescription)
        }

        guard let identity else {
            // 401: the credential is already dead server-side (a prior
            // revocation, or the device was removed from another client).
            envFile.delete()
            store.suspended = nil
            return .alreadyRevoked
        }
        ...
        // Revoked server-side: the device row is gone and every live control
        // and bridged connection was severed. Drop the local credential so a
        // restart can't dial with it, and remember the machine so this same
        // account can get it back by signing in.
        envFile.delete()
    ...
    private func retryPendingCore() async {
        ...
        switch outcome {
        case .unreachable:
            // Still can't reach the relay. Stay armed.
            observeNetworkIfNeeded()
    ...
    private func restoreCore(accountEmail: String, sessionToken: String) async {
        // A pending revocation for THIS account is cancelled rather than
        // completed: the account that walked away came back to the same
        // machine, and the credential it left behind never left this Mac.
        if let pending = store.pendingRevocationEmail {
            if pending.caseInsensitiveCompare(accountEmail) == .orderedSame {
                store.pendingRevocationEmail = nil
                return
            }
            // A DIFFERENT account is signing in. The previous account's machine
            // must still go away before this one takes over the host.
