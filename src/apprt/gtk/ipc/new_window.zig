...
//
// `ghoztty +new-window` is equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest com.dzearing.ghoztty --object-path /com/dzearing/ghoztty --method org.gtk.Actions.Activate new-window [] []
// ```
//
// `ghoztty +new-window -e echo hello` would be equivalent to the following command (on a release build):
//
// ```
// gdbus call --session --dest com.dzearing.ghoztty --object-path /com/dzearing/ghoztty --method org.gtk.Actions.Activate new-window-command '[<@as ["-e" "echo" "hello"]>]' []
// ```
